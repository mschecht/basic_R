...
With the function `c()` we can generate strings of values. With the coordinates (square brackets, `[` and `]`) we can select specific elements from strings. With the operator `$` we can access columns in a data frame by the variable name. The double square bracket operator (`[[` and `]]`) is useful for lists.
...
Check the [R markdown cheatsheet](https://www.rstudio.com/resources/cheatsheets/).
...
```{r}
#load("D:/Workshops/201901_R_workshop/R_workshop.RData")
```
### Working directories
